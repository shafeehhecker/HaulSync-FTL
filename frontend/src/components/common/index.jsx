...
export function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium tracking-wide ${cls}`}
    >
      {label}
    </span>
  );
...
export function Modal({ open, onClose, title, children, width = "max-w-lg" }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      />
      <div
        className={`relative ${width} w-full bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl animate-fade-in`}
      >
...
export function InfoBanner({ message, type = "info" }) {
  ...
  return (
    <div
      className={`flex items-center gap-2 px-4 py-3 rounded-lg border text-sm ${styles[type] || styles.info}`}
    >
