...
import { listOf, mapTrip } from '../../api/normalize';

const MOCK_TRIPS = [
  { id: '1', tripNumber: 'TRIP-2025-0841', origin: 'Mumbai', dest: 'Delhi', driver: 'Rajan Kumar', vehicle: 'MH-04-AB-1234', status: 'IN_TRANSIT', lat: 21.1458, lng: 79.0882, location: 'Nagpur bypass, NH-44', speed: 68, lastPing: '2 min ago', eta: '2h 30m', progressPct: 30, gpsProvider: 'Vamosys' },
  { id: '2', tripNumber: 'TRIP-2025-0840', origin: 'Pune', dest: 'Bangalore', driver: 'Suresh Yadav', vehicle: 'MH-12-XY-5678', status: 'AT_PICKUP', lat: 18.5204, lng: 73.8567, location: 'Pune Loading Dock', speed: 0, lastPing: '1 min ago', eta: '—', progressPct: 0, gpsProvider: 'Locus' },
  { id: '3', tripNumber: 'TRIP-2025-0839', origin: 'Delhi', dest: 'Kolkata', status: 'DELAYED', driver: 'Mohan Singh', vehicle: 'DL-01-CD-9012', lat: 25.3176, lng: 82.9739, location: 'Varanasi — extended halt', speed: 0, lastPing: '8 min ago', eta: '5h 10m', progressPct: 44, gpsProvider: 'Vamosys' },
  { id: '4', tripNumber: 'TRIP-2025-0838', origin: 'Chennai', dest: 'Hyderabad', driver: 'Anil Sharma', vehicle: 'TN-01-EF-3456', status: 'DELIVERED', lat: 17.3850, lng: 78.4867, location: 'Hyderabad Delivery Point', speed: 0, lastPing: '45 min ago', eta: '—', progressPct: 100, gpsProvider: 'Locus' },
];
...
export default function TrackingPage() {
  ...
  useEffect(() => {
    api.get('/ftl/tracking/active')
      .then(r => {
        const list = listOf(r.data).map(mapTrip);
        setTrips(list);
        setSelected(list[0] ?? null);
      })
      .catch(() => { setTrips(MOCK_TRIPS.map(mapTrip)); setSelected(mapTrip(MOCK_TRIPS[0])); })
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-1 space-y-2">
          {filtered.length === 0 && (
            <div className="card p-6 text-center text-sm text-zinc-500">No active trips in this state.</div>
          )}
          {filtered.map(trip => (
          ...
          <div className="lg:col-span-2 space-y-4">
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 relative">
                    <div className="h-1 bg-zinc-800 rounded-full">
                      <div className="h-full bg-amber-400 rounded-full transition-all" style={{ width: `${selected.progressPct}%` }} />
                    </div>
                    <div className="absolute top-1/2 -translate-y-1/2 transition-all" style={{ left: `${selected.progressPct}%` }}>
                      <div className={`w-3 h-3 rounded-full border-2 border-zinc-900 ${statusColor[selected.status] || 'bg-zinc-400'}`} />
                    </div>
                    <p className="text-center text-[10px] text-zinc-500 mt-2">{selected.progressPct}% of route</p>
                  </div>
                  <div className="text-center">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                { label: 'Progress', value: `${selected.progressPct}%`, icon: MapPin, color: 'purple' },
